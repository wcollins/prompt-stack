---
description: Sync with origin and delete feature branch after PR merge
---

# Reset After Merge (Trunk Workflow)

Clean up after a PR is merged and prepare for the next task.

Use this command after your pull request has been merged.
...
- If already on `main`: skip to step 3 (sync with origin)
...
> Reset complete. You're on `main`, synced with origin.
>
> Deleted branch `<branch-name>` (local and remote).
>
> Ready for next task: `/branch-trunk <task>`

If already on main (no branch to delete):

> Synced `main` with origin.
>
> Ready for next task: `/branch-trunk <task>`
