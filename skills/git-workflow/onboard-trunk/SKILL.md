---
description: Stage and commit an uncommitted codebase in logical chunks via PRs to origin
argument-hint: "[continue]"
---

# Onboard Codebase (Trunk Workflow)

Stage and commit an uncommitted codebase in logical chunks via PRs to origin.

Use this command to version control an existing project that has code but no meaningful commit history. It breaks the codebase into reviewable chunks and creates separate PRs for each.

**Usage:**
- `/onboard-trunk` - Initial run: analyze codebase, create chunk plan, stage first chunk
- `/onboard-trunk continue` - After PR merge: stage next chunk
...
- Jump to **Continue Flow** section (step 12)
...
> - To continue: `/onboard-trunk continue`
...
> - ... (showing first 10)
>
> **Note:** `/pr-trunk` will create individual commits for each file.
> Commit type for this chunk: `<type>`
>
> **Next steps:**
> 1. Review staged files: `git diff --staged`
> 2. Create PR: `/pr-trunk` (creates one commit per file)
> 3. Merge PR in GitHub
> 4. Clean up: `/reset-trunk`
> 5. Continue: `/onboard-trunk continue`
...
>
> Run `/onboard-trunk` to start a new session.

Parse the state file and validate:
- workflow should be "trunk"
- phase should be "awaiting_pr"
- current_chunk_index should be valid

### 13. Verify Previous Chunk Complete
...
> 1. Run `/pr-trunk` if you haven't created the PR
> 2. Merge the PR in GitHub
> 3. Run `/reset-trunk` to clean up
> 4. Then run `/onboard-trunk continue`

### 14. Mark Previous Chunk Complete
...
### 15. Check if All Chunks Complete
...
### 17. Create Branch for Next Chunk
...
- **Do NOT create commits** - user runs `/pr-trunk` for that
- **Do NOT push to remote** - user runs `/pr-trunk` for that
- **Do NOT modify the pr-trunk or reset-trunk commands** - they work as-is
- **One file = one commit** - `/pr-trunk` creates individual commits per file
